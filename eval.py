    ...
    for pic_dir in tqdm(os.listdir(gt_dir), desc="mAP evaluation progress"):
        ...
        for fname in os.listdir(gt_dir / pic_dir):
            ...
            for threshold in threshold_values:
                ...
                #print(msk.shape)torch.Size([1, 520, 779])
                precision_value = precision(msk, gt)
                #print(precision_value)tensor(0.2271)
            ...
            #print(precision_list.shape)(101,)
...
def rec_evaluate(renders, gts, json_pth=None):
    ...
    mean_ssim = torch.tensor(ssims).mean().tolist()
    mean_psnrs = torch.tensor(psnrs).mean().tolist()
    mean_lpips = torch.tensor(lpipss).mean().tolist()

    print("  SSIM : {:>12.7f}".format(mean_ssim, ".5"))
    print("  PSNR : {:>12.7f}".format(mean_psnrs, ".5"))
    print("  LPIPS: {:>12.7f}".format(mean_lpips, ".5"))

    if json_pth:
        with open(json_pth, "w") as f:
            json.dump({"SSIM": mean_ssim, "PSNR": mean_psnrs, "LPIPS": mean_lpips}, f, indent=4)
...
def lem_evaluate(renders, gts, json_pth=None):
    ...
    means_iou = np.mean(IoUs_list)
    means_acc = np.mean(accuracies_list)
    means_prec = np.mean(precisions_list)

    print("  mIoU : {:>12.7f}".format(means_iou, ".5"))
    print("  accuracy : {:>12.7f}".format(means_acc, ".5"))
    print("  precision : {:>12.7f}".format(means_prec, ".5"))
    
    if json_pth:
        with open(json_pth, "w") as f:
            json.dump({"mIoU": means_iou,"accuracy": means_acc,"precision": means_prec,
                       "IoUs": IoUs, "accuracies": accuracies, "precisions": precisions}, f, indent=4)
...
if __name__ == "__main__":
    ...
    #print(eval_dir)output/mipnerf360/kitchen/0/open_new_eval_softmax_s10.0_a05
    ...
    rec_evaluate(renders, gts, eval_dir / "rec_metrics.json")
